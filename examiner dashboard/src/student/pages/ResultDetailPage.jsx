...
export default function ResultDetailPage() {
  ...
  if (!result) return <StudentShell><section className="student-page-title"><h1>Result unavailable</h1><p>This result has not been published or is no longer available.</p><button type="button" onClick={() => navigate('/student/results')}>Back to Results</button></section></StudentShell>;
  ...
  return (
    <StudentShell>
      ...
      <section className="student-shell detail-hero">
        <article><span><UserRound size={64} /></span><h2>{result.candidateName || profile.name}</h2><p><Award size={18} /> {result.subject} - {result.grade} <CalendarDays size={18} /> {result.live ? result.date : 'Summer Term 2024'} <BadgeCheck size={18} /> ID: {result.registration || 'CN-882910'}</p></article>
        <aside><small>Final Grade</small><strong>{score}</strong><em>{classification}</em></aside>
      </section>
      {result.live ? <section className="student-shell detail-grid">
        <article className="technical-card"><h2>Technical</h2><p>{result.feedback?.technical || 'No technical feedback recorded.'}</p></article>
        <article className="repertoire-card"><h2>Artistic Interpretation</h2><p>{result.feedback?.artistic || 'No artistic feedback recorded.'}</p></article>
        <article className="support-card"><h2>Assessment</h2><p>{result.subject} - {result.grade}</p><p>{result.date}</p><p>{result.registration}</p></article>
        <article className="narrative-card"><h2>Examiner's Narrative</h2><p>{result.feedback?.overall || 'No overall feedback recorded.'}</p></article>
      </section> : <section className="student-shell detail-grid">
      ...
      <div className="detail-actions">
        <button
          ...
          type="button"
          onClick={() => {
            const ok = makePdfDownload(`${result.id}-certificate.pdf`, ['Official Certificate', result.candidateName || profile.name, `${result.subject} - ${result.grade}`, `Score: ${score}`, classification, result.date, `CERT-${result.id.toUpperCase()}`]);
